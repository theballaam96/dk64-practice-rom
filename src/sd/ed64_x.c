...
#include "../../include/ed/ed64_x.h"
#include "../../include/ed/iodev.h"
#include "../../include/ed/pi.h"
#include "../../include/ultra64/pi.h"
#include "../../include/ed/sd_host.h"
#include "../../include/ed/util.h"
#include "../../include/ultra64.h"

static int      cart_irqf;
static unsigned int cart_lat;
static unsigned int cart_pwd;
static unsigned short spi_cfg;
...
static inline unsigned int reg_rd(int reg)
{
  return __pi_read_raw((unsigned int)&REGS_PTR[reg]);
}

static inline void reg_wr(int reg, unsigned int dat)
{
  return __pi_write_raw((unsigned int)&REGS_PTR[reg], dat);
}
...
static inline void cmd_tx(unsigned char dat)
{
...
static inline unsigned char cmd_rx(void)
{
...
static inline void dat_tx(unsigned short dat)
{
...
static inline unsigned short dat_rx(void)
{
...
static int fifo_read(void *dst, size_t n_blocks)
{
  ...
  while (n_blocks != 0) {
    ...
    pi_read_locked((unsigned int)&REGS_PTR[REG_USB_DAT], p, blk_size);
...
static int fifo_write(const void *src, size_t n_blocks)
{
  ...
  while (n_blocks != 0) {
    ...
    pi_write_locked((unsigned int)&REGS_PTR[REG_USB_DAT], p, blk_size);
