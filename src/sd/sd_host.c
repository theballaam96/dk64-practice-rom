#include "../../include/ed/io.h"
#include "../../include/ed/sd.h"
...
static void crc16_wide(const void *data, int size, unsigned short (*crc_buf)[4])
{
  ...
  unsigned char *crc_p = (void *)*crc_buf;
  for (int i = 0; i < 8; i++) {
    *crc_p++ = crc >> 56;
    crc = crc << 8;
  }
...
static int cs_err(unsigned int cs)
{
  if ((cs & CS_ERR_BITS) == 0)
    return 0;
...
static int card_cmd_sd(struct sd_host *host, int cmd, const unsigned char *tx_buf,
...
static int card_cmd_spi(struct sd_host *host, int cmd, const unsigned char *tx_buf,
                        unsigned char *rx_buf)
{
...
static int card_cmd(struct sd_host *host, int cmd, unsigned int arg, void *resp)
{
...
int sd_init(struct sd_host *host)
{
  ...
  for (;;) {
    ...
    /* 3.2V - 3.4V */
    unsigned int op_cond = OCR_3V2_3V3 | OCR_3V3_3V4;
